...
export default function AdminChatPage() {
  ...
  const handleSendMessage = async (e) => {
    ...
    } else if (imagePreview && cameraInputRef.current?.files?.[0]) {
      imageUrl = await handleImageUpload(cameraInputRef.current.files[0]);
    }
    ...
    <div className="flex h-[calc(100vh-120px)] bg-gray-100 rounded-lg overflow-hidden">
      ...
      <div className="w-80 bg-white border-r border-gray-200 flex flex-col">
        <div className="p-4 border-b border-gray-200">
          <h2 className="text-lg font-semibold text-gray-800">Conversații</h2>
        </div>
        ...
        >
          ...
          <div className="flex-1">
            <p className="font-medium text-gray-800">Chat General</p>
            <p className="text-sm text-gray-500">{onlineAdmins.length} admini online</p>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          {otherAdmins.map(admin => (
            <div 
              key={admin.id}
              onClick={() => setSelectedChat(admin.id)}
              className={`p-4 cursor-pointer hover:bg-gray-50 transition-colors flex items-center gap-3 ${
            ...
            >
              <div className="relative">
                {admin.image ? (
                  ...
                  <div className={`w-12 h-12 rounded-full bg-gradient-to-br ${getAvatarColor(admin.email)} flex items-center justify-center text-white font-bold text-lg shadow-md`}>
              ...
              <div className="flex-1 min-w-0">
                <p className="font-medium text-gray-800 truncate">{admin.name || admin.email}</p>
        ...
        <div className="p-4 bg-white border-b border-gray-200 flex items-center gap-3">
          {selectedChat === 'general' ? (
            <>
              ...
              <div>
                <h3 className="font-semibold text-gray-800">Chat General</h3>
                <p className="text-sm text-gray-500">{onlineAdmins.length} administratori online</p>
              </div>
            </>
          ) : (
            <>
                ...
                return admin ? (
                  <>
                    {admin.image ? (
                      ...
                      <div className={`w-10 h-10 rounded-full bg-gradient-to-br ${getAvatarColor(admin.email)} flex items-center justify-center text-white font-bold shadow-md`}>
                        {getInitials(admin.name, admin.email)}
                      </div>
                    )}
                    <div>
                      <h3 className="font-semibold text-gray-800">{admin.name || admin.email}</h3>
                      <p className="text-sm">
                        {isOnline(admin.email) ? (
                          <span className="text-green-600">Online</span>
                        ) : (
            ...
            <div key={date}>
              ...
              {dayMessages.map((message) => {
                ...
                return (
                  <div
                    key={message.id}
                    className={`flex items-end gap-2 mb-3 ${isOwnMessage ? 'flex-row-reverse' : ''}`}
                  >
                    {/* Avatar */}
                    {!isOwnMessage && (
                      message.senderImage ? (
                        ...
                        <div className={`w-8 h-8 rounded-full bg-gradient-to-br ${getAvatarColor(message.senderEmail)} flex items-center justify-center text-white text-xs font-bold shadow-md`}>
                    ...
                    {/* Message bubble */}
                    <div className={`max-w-xs md:max-w-md lg:max-w-lg ${isOwnMessage ? 'items-end' : 'items-start'}`}>
                      {!isOwnMessage && (
        ...
        <form onSubmit={handleSendMessage} className="p-4 bg-white border-t border-gray-200">
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="p-2 text-gray-500 hover:text-amber-600 hover:bg-gray-100 rounded-full transition-colors"
            ...
            <input
              ...
              placeholder={`Mesaj în ${getSelectedChatName()}...`}
              className="flex-1 px-4 py-2 border border-gray-300 rounded-full focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500"
            />
