...
export async function PATCH(request, { params }) {
  try {
    ...
    if (!hasAccess) {
      return NextResponse.json({ error: "Acces interzis. Doar administratorii pot modifica produse." }, { status: 403 })
    }
...
export async function DELETE(request, { params }) {
  try {
    ...
    const hasAccess = await checkFullAdminAccess(session)
    if (!hasAccess) {
      return NextResponse.json({ error: "Acces interzis. Doar administratorii pot șterge produse." }, { status: 403 })
    }
